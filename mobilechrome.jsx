...
(function () {
  ...
  function ptsLottieDataC() {
    if (!ptsLottiePromiseC) {
      ptsLottiePromiseC = fetch(PTS_LOTTIE_SRCC).then((r) => r.ok ? r.json() : null).catch(() => { ptsLottiePromiseC = null; return null; });
    }
  ...
  function ptsReadFullC() {
    const eng = window.PFLoyalty;
    if (eng && eng.getConfig) { try { return Math.max(1, +eng.getConfig().beakerFullPoints || PTS_FULL_FALLBACKC); } catch (e) { /* fall through */ } }
  ...
  function PointsIconC() {
    ...
    return (
      <span className="m-pts-ic" aria-hidden="true">
        {!ready && <span className="m-pts-ic-fb"><DSC.IconifyIcon name="lucide:smile" size={18} color="#FCC25D" /></span>}
  ...
  function usePointsCountUpC(target) {
    ...
    useEffectC(() => {
      ...
      if (ptsReduceMotionC() || typeof requestAnimationFrame !== "function") { from.current = target; setShown(target); return; }
  ...
  function ptsPopupOnC() { try { const s = JSON.parse(localStorage.getItem("pf-gamification")) || {}; return s.pointsPopup !== false; } catch (e) { return true; } }
  function PointsTallyC({ host, on, shown, bump, delta }) {
    return ReactDOM.createPortal(
      <div className={"m-pts-tally" + (on ? " on" : "")} aria-hidden="true">
        <button type="button" tabIndex={-1} className={"m-pts" + (delta && on ? " earn" : "")} onClick={() => goC("RewardsDashboard.html")}>
          <PointsIconC />
          <span key={bump} className={"m-pts-n" + (bump ? " pop" : "")}>{shown.toLocaleString("en-GB")}</span>
          {delta && on && <span key={delta.key} className="m-pts-delta" aria-hidden="true">+{delta.amt}</span>}
        </button>
      </div>, host);
  }
  ...
  function PointsPillC() {
    ...
    useEffectC(() => {
      const refresh = () => { setTotal(ptsReadTotalC()); setFull(ptsReadFullC()); };
      const onEarn = (e) => {
        ...
        if (eng) {
          try {
            ...
            if (e.detail.booked) { /* already in the engine */ }
      ...
      const onStorage = (e) => { if (!e.key || e.key === "pf-loyalty-state-v1" || e.key === "pf-loyalty-config-v1") refresh(); };
      window.addEventListener("pf:points-earned", onEarn);
      window.addEventListener("storage", onStorage);
      return () => { window.removeEventListener("pf:points-earned", onEarn); window.removeEventListener("storage", onStorage); clearTimeout(tallyTimer.current); };
    }, []);
    ...
    return (
      <React.Fragment>
        <button ref={pillRef} type="button" className={"m-pts" + (delta ? " earn" : "")} aria-label={label} title={total.toLocaleString("en-GB") + " / " + full.toLocaleString("en-GB") + " pts"}
          onClick={() => goC("RewardsDashboard.html")}>
          <PointsIconC />
          <span key={bump} className={"m-pts-n" + (bump ? " pop" : "")}>{shown.toLocaleString("en-GB")}</span>
          {delta && <span key={delta.key} className="m-pts-delta" aria-hidden="true">+{delta.amt}</span>}
        </button>
  ...
  function MTopBarC({ onMenu, onBell, onMessages, dark }) {
    return (
      <header className="m-top">
        ...
        <span className="grow" />
        <PointsPillC />
        <button className="m-iconbtn" aria-label="Search" onClick={() => goC("SearchMobile.html")}><DSC.Icon name="search" size={20} color="var(--brand-navy)" /></button>
...
function avatarNameC(name) { return String(name || "").replace(/^(dr\.?|nurse|prof\.?|mr\.?|mrs\.?|ms\.?)\s+/i, ""); }
      ...
      <span className="mp-group-av" style={{ width: s, height: s }}>
        {members.slice(0, 2).map((m, i) =>
          <span className="mp-group-av-item" key={m.id || i}>
            <DSC.Avatar name={avatarNameC(m.name)} src={m.avatar} size={Math.round(s * 0.68)} />
          </span>
        )}
      </span>);
  }
      ...
      <button className="mp-row" data-thread-id={c.id} onClick={onOpen}>
        <span className="mp-av">
          {c.isGroup ?
            ...
            <>
              <DSC.Avatar name={avatarNameC(c.name)} src={c.avatar} size={52} />
          ...
          {filtered.map((c) => {
            ...
            return (
              <button key={c.id} className={"mp-new-row" + (on ? " on" : "")} onClick={() => onToggle(c.id)}>
                <span className="mp-av"><DSC.Avatar name={avatarNameC(c.name)} src={c.avatar} size={44} /></span>
      ...
      return (
        <div className={"m-drawer-wrap" + (open ? " open" : "")} aria-hidden={!open}>
          ...
          <aside className="m-drawer nt-panel mp-panel" role="dialog" aria-modal="true" aria-label="New Conversation">
            <NewConversationScreenC contacts={DM_THREADS_SEED_C.filter((c) => !c.isGroup)} picked={picked} onToggle={togglePick}
    ...
    return (
      <div className={"m-drawer-wrap" + (open ? " open" : "")} aria-hidden={!open}>
        ...
        <aside className="m-drawer nt-panel mp-panel" role="dialog" aria-modal="true" aria-label="Messages">
          ...
          <button className="mp-expand" aria-label="Open Messages" onClick={() => goC("Messages.html")}>
            <DSC.IconifyIcon name="lucide:maximize-2" size={20} color="var(--gray-900)" />
          </button>
