  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10, background: "var(--surface-card)", border: "1px solid var(--border-default)", borderRadius: "var(--r-md)", boxShadow: "var(--shadow-card)", padding: 16 }}>
      ...
      <WebGoLiveStage description={liveDescription} onDescriptionChange={setLiveDescription}
        onClose={() => setLiveStage(null)} onGoLive={() => setLiveStage("live")} />}
...
function WebGoLiveStage({ description, onDescriptionChange, onGoLive, onClose }) {
  ...
  return (
    <div className="pfw-bcast" role="dialog" aria-modal="true" aria-label="Live broadcast">
      ...
      <div className="pfw-bcast-chat" ref={chatRef} onScroll={onChatScroll}>
        {chat.map((c, i) => (
          <div className={"pfw-bcast-msg" + (c.me ? " me" : "") + (c.fresh ? " in" : "")} key={i}><b>{c.n}</b> {c.t}</div>
        ))}
      </div>
