...
const PROFINITY_BY = { by: "Profinity", byAvatar: "assets/profinity-icon.jpg", avatar: "assets/profinity-icon.jpg" };
const TIM_BY = { by: "Dr Tim Pearce", byAvatar: "assets/avatar-drtim.png", avatar: "assets/profinity-icon.jpg" };
const TIM_LOCAL = { ...TIM, avatar: "assets/avatar-drtim.png" };
const CONFIDENCE_PINNED_1 = {
  id: "chpin_confidence_1", access: "gated", bucket: "confidence",
  author: PROFINITY, keepAuthor: true,
  channel: { name: "#Confidence · Community", ...PROFINITY_BY, time: "1w" },
  time: "1 Week Ago", hashtags: ["confidence", "community"],
  media: [IMG.communityPoster],
  ...
  likes: "1.4K", comments: "236", shares: "58", actioned: false,
  commentList: [
  { author: { name: "Nurse Beth", avatar: "assets/avatar-nurse-beth.jpg" }, text: "Hello from Leeds! Six months in and this channel has already saved me twice 🙌",
    likes: "84", comments: "3", time: "6d", pills: [{ k: "like", n: "60" }, { k: "love", n: "24" }], reactions: ["like", "love"], reactionCount: "84" }]
};
const CONFIDENCE_PINNED_2 = {
  id: "chpin_confidence_2", access: "gated", bucket: "confidence",
  author: TIM_LOCAL, keepAuthor: true,
  channel: { name: "#Confidence · Community", ...TIM_BY, time: "2w" },
  time: "2 Weeks Ago", hashtags: ["confidence", "guidelines"],
  ...
  likes: "982", comments: "71", shares: "112", actioned: false, commentList: []
};
const MASTERY_PINNED_1 = {
  id: "chpin_mastery_1", access: "gated", bucket: "mastery",
  author: TIM_LOCAL, keepAuthor: true,
  channel: { name: "#Mastery · Community", ...TIM_BY, time: "3d" },
  time: "3 Days Ago", hashtags: ["mastery", "casereview"],
  media: [IMG.communityPoster],
  ...
  likes: "640", comments: "58", shares: "31", actioned: false,
  commentList: [
  { author: { name: "Priya Shah", avatar: "assets/avatar-priya-shah.jpg" }, text: "Tear trough on a 52yo with prior HA — cannula entry point debate. In!",
    likes: "22", comments: "1", time: "2d", pills: [{ k: "like", n: "22" }], reactions: ["like"], reactionCount: "22" }]
};
const MASTERY_PINNED_2 = {
  id: "chpin_mastery_2", access: "gated", bucket: "mastery",
  author: PROFINITY, keepAuthor: true,
  channel: { name: "#Mastery · Community", ...PROFINITY_BY, time: "1w" },
  time: "1 Week Ago", hashtags: ["mastery", "howto"],
  ...
  likes: "455", comments: "24", shares: "40", actioned: false, commentList: []
};
const FREEDOM_PINNED_1 = {
  id: "chpin_freedom_1", access: "gated", bucket: "freedom",
  author: TIM_LOCAL, keepAuthor: true,
  channel: { name: "#Freedom · Community", ...TIM_BY, time: "2d" },
  time: "2 Days Ago", hashtags: ["freedom", "growthsprint"],
  media: [IMG.communityPoster],
  ...
  likes: "318", comments: "49", shares: "17", actioned: false,
  commentList: [
  { author: { name: "Dr Amir Khan", avatar: "assets/avatar-amir-khan.jpg" }, text: "In. Clinic three opens in March so the timing is perfect.",
    likes: "15", comments: "0", time: "1d", pills: [{ k: "like", n: "15" }], reactions: ["like"], reactionCount: "15" }]
};
const FREEDOM_PINNED_2 = {
  id: "chpin_freedom_2", access: "gated", bucket: "freedom",
  author: PROFINITY, keepAuthor: true,
  channel: { name: "#Freedom · Community", ...PROFINITY_BY, time: "1w" },
  time: "1 Week Ago", hashtags: ["freedom", "resources"],
  ...
  likes: "276", comments: "12", shares: "63", actioned: false, commentList: []
};
const INNER_PINNED_1 = {
  id: "chpin_inner_1", access: "gated", bucket: "inner",
  author: TIM_LOCAL, keepAuthor: true,
  channel: { name: "#Inner Circle · Community", ...TIM_BY, time: "4d" },
  time: "4 Days Ago", hashtags: ["inner-circle", "retreat"],
  media: [IMG.communityPoster],
  ...
  likes: "188", comments: "27", shares: "6", actioned: false,
  commentList: [
  { author: { name: "Dr Rachel Voss" }, text: "Confirmed. Flying in Thursday night — anyone else on the early flight?",
    likes: "9", comments: "2", time: "3d", pills: [{ k: "like", n: "9" }], reactions: ["like"], reactionCount: "9" }]
};
const INNER_PINNED_2 = {
  id: "chpin_inner_2", access: "gated", bucket: "inner",
  author: PROFINITY, keepAuthor: true,
  channel: { name: "#Inner Circle · Community", ...PROFINITY_BY, time: "2w" },
  time: "2 Weeks Ago", hashtags: ["inner-circle", "houserules"],
  ...
  likes: "142", comments: "8", shares: "3", actioned: false, commentList: []
};
...
const BUCKET_POSTS = [
CONFIDENCE_PINNED_1, CONFIDENCE_PINNED_2,
...
MASTERY_PINNED_1, MASTERY_PINNED_2,
...
FREEDOM_PINNED_1, FREEDOM_PINNED_2,
...
function popWrong() {
  if (typeof window === "undefined") return;
  try { window.dispatchEvent(new CustomEvent("pf:answer-wrong")); } catch (e) { /* older WebView */ }
}
...
function PostComposer({ onPost, superUser, devRole, onDevRole, lockedAdmin }) {
  ...
  const confirmSchedule = ({ title, startIso }) => {
    const item = {
      id: "sl" + Date.now(), title, startIso, dest: "Newsfeed",
      host: { name: ME.name, avatar: ME.avatar }, createdAt: new Date().toISOString()
    };
  ...
  const pickImages = () => {
    ...
    input.onchange = (e) => {
      Array.from(e.target.files || []).slice(0, Math.max(0, 5 - images.length)).forEach((f) => {
        window.pfReadImageFile(f).then((src) => { if (src) setImages((prev) => [...prev, src].slice(0, 5)); });
      });
    };
    input.click();
  };
  ...
  const pickVideo = () => {
    ...
    input.onchange = (e) => {
      ...
      probe.addEventListener("loadedmetadata", () => {
        ...
        setVideo((cur) => cur && ({ ...cur, ratio, duration: probe.duration }));
      }, { once: true });
    };
    input.click();
  };
  ...
  const removeVideo = () => {
    if (video && video.src) { try { URL.revokeObjectURL(video.src); } catch (e) {} }
    setVideo(null);
  };

  const handleCoverConfirm = (selected) => {
    setVideo((cur) => cur && ({ ...cur, cover: selected.src, coverIsCustom: !!selected.custom }));
  ...
  const endLive = (keepReplay) => {
    ...
    if (keepReplay && liveDescription.trim()) {
      onPost({ body: liveDescription.trim(), media: [], video: null, live: true });
    }
    setLiveDescription("");
  };
    ...
    <div className="pfw-composer-card">
      <div className="pfw-composer-row">
        ...
        <div className="pfw-composer-quick">
          <ComposerIconButton icon="fluent:video-24-filled" color="#E8455D" label="Add video" onClick={openModalAnd(pickVideo)} disabled={!!video} />
          <ComposerIconButton icon="fluent:image-multiple-24-filled" color="#3DBE5B" label="Add photo" onClick={openModalAnd(pickImages)} disabled={images.length >= 5} />
      ...
      <WebGoLiveStage description={liveDescription} onDescriptionChange={setLiveDescription}
        onClose={() => setLiveStage(null)} onGoLive={startLive} onSchedule={() => setSchedModal(true)} />}

      {schedModal &&
      <WebScheduleLiveModal defaultTitle={liveDescription} onConfirm={confirmSchedule} onClose={() => setSchedModal(false)} />}

      {scheduled &&
      <WebScheduledConfirm item={scheduled} onDone={() => setScheduled(null)}
        onViewProfile={() => (window.pfGo || function (u) { window.location.href = u; })("Profile.html#upcoming-lives")} />}
...
function CreatePostModal({ v, setV, images, setImages, video, categories, setCategories, poll, setPoll, togglePoll, doc, setDoc, pickDocument, location, setLocation, bg, pickBg, pickImages, pickVideo, removeVideo, submit, ready, firstName, coverPickerOpen, setCoverPickerOpen, handleCoverConfirm, superUser, devRole, onDevRole, lockedAdmin, onGoLive, onClose }) {
  ...
  const options = [
    { key: "photo", icon: "lucide:image", label: "Photo", color: "#2d9d5a", onClick: pickImages, disabled: !!poll || !!bg || images.length >= 5 },
    { key: "camera", icon: "lucide:camera", label: "Camera", color: "#292569", onClick: () => setCameraOpen(true), disabled: !!poll || !!bg || images.length >= 5 },
    { key: "video", icon: "lucide:video", label: "Video", color: "#c8323a", onClick: pickVideo, disabled: !!video || !!poll || !!bg },
...
function WebCoverPicker({ video, onConfirm, onClose }) {
  ...
  useEffect(() => {
    ...
    const build = async () => {
      await new Promise((resolve) => {
        if (vEl.readyState >= 1) resolve();
        else vEl.addEventListener("loadedmetadata", resolve, { once: true });
      });
      if (cancelled) return;
  ...
  return (
    <div className="pfw-cover-overlay" onClick={onClose}>
      <div className="pfw-cover-modal" role="dialog" aria-modal="true" aria-label="Edit cover"
        onClick={(e) => e.stopPropagation()}>
        <video ref={hiddenVideoRef} src={video.src} muted playsInline style={{ display: "none" }} />
        <canvas ref={canvasRef} style={{ display: "none" }} />

        <header className="pfw-cover-top">
          <span className="pfw-cover-title">Edit cover</span>
          <button type="button" className="pfw-cover-x" aria-label="Cancel" onClick={onClose}>
            <IconifyIcon name="lucide:x" size={18} color="var(--text-heading)" />
          </button>
        </header>

        <p className="pfw-cover-hint">Select a cover image from your video, or upload your own.</p>

        <div className="pfw-cover-preview">
          {selected ? <img src={selected.src} alt="" /> : <span className="pfw-cover-loading">Loading frames…</span>}
        </div>

        <div className="pfw-cover-filmstrip" role="radiogroup" aria-label="Video frame">
          {frames.map((f, i) => (
          ...
          {building && frames.length < PFW_COVER_FRAME_COUNT &&
            Array.from({ length: PFW_COVER_FRAME_COUNT - frames.length }).map((_, i) => (
              <span className="pfw-cover-frame-skel" key={"s" + i} aria-hidden="true" />
            ))}
        </div>

        <div className="pfw-cover-actions">
          <button type="button" className="pfw-cover-roll-btn" onClick={pickFromDisk}>
            <IconifyIcon name="lucide:image" size={16} color="var(--brand-navy)" />
            Upload a photo instead
          </button>
          <button type="button" className="pfw-cover-check" disabled={!selected}
...
function pfwLoadScheduled() {
  try { return JSON.parse(localStorage.getItem(PFW_SCHED_KEY)) || []; } catch (e) { return []; }
}
function pfwSaveScheduled(list) {
  try { localStorage.setItem(PFW_SCHED_KEY, JSON.stringify(list)); } catch (e) {}
}
...
function WebScheduleLiveModal({ defaultTitle, onConfirm, onClose }) {
  ...
  const submit = () => {
    ...
    if (!when || isNaN(d)) { setErr("Pick a date and time."); return; }
    if (d.getTime() < Date.now() + 5 * 60 * 1000) { setErr("Choose a time at least 5 minutes from now."); return; }
    onConfirm({ title: title.trim() || "Live with " + ME.name, startIso: d.toISOString() });
  };
  return (
    <div className="pfw-cover-overlay pfw-sched-overlay" onClick={onClose}>
      <div className="pfw-cover-modal pfw-sched-modal" role="dialog" aria-modal="true" aria-label="Schedule live" onClick={(e) => e.stopPropagation()}>
        <div className="pfw-cover-top">
          <span className="pfw-cover-title">Schedule live</span>
          <button type="button" className="pfw-cover-x" aria-label="Close" onClick={onClose}>
            <IconifyIcon name="lucide:x" size={16} color="var(--gray-600)" />
          </button>
        </div>
        <p className="pfw-cover-hint">Your followers will see it on your profile under <b>Upcoming lives</b> — you can go live from there when it's time.</p>
        <div className="pfw-sched-body">
          <label className="pfw-sched-field">
            <span>Title</span>
            <input type="text" value={title} maxLength={80} autoFocus placeholder="e.g. Live Q&A: correcting migrated lip filler"
              onChange={(e) => setTitle(e.target.value)} />
          </label>
          <label className="pfw-sched-field">
            <span>Date &amp; time</span>
            <input type="datetime-local" value={when} min={pfwToLocalInput(new Date())}
              onChange={(e) => { setWhen(e.target.value); setErr(""); }} />
          </label>
          <div className="pfw-sched-dest"><IconifyIcon name="lucide:rss" size={15} color="var(--gray-500)" />Streaming to <b>Newsfeed</b></div>
          {err && <p className="pfw-sched-err" role="alert">{err}</p>}
        </div>
        <div className="pfw-cover-actions">
          <button type="button" className="pfw-cover-roll-btn" onClick={onClose}>Cancel</button>
          <button type="button" className="pfw-cover-check" onClick={submit}>
            <IconifyIcon name="lucide:calendar-clock" size={16} color="#fff" style={{ marginRight: 6 }} />
...
function WebScheduledConfirm({ item, onViewProfile, onDone }) {
  useEffect(() => {
  ...
  return (
    <div className="pfw-cover-overlay pfw-sched-overlay" onClick={onDone}>
      <div className="pfw-sched-confirm" role="dialog" aria-modal="true" aria-label="Live scheduled" onClick={(e) => e.stopPropagation()}>
        <span className="pfw-sched-confirm-ic"><IconifyIcon name="lucide:calendar-check" size={30} color="var(--success)" /></span>
        <h3>Live scheduled</h3>
        <p className="ti">{item.title}</p>
        <p className="tm"><IconifyIcon name="lucide:clock" size={14} color="var(--gray-500)" />{pfwFormatWhen(item.startIso)} · {item.dest}</p>
        <p className="note">It's now listed under <b>Upcoming lives</b> on your profile.</p>
        <div className="pfw-sched-confirm-actions">
          <button type="button" className="pfw-cover-roll-btn" onClick={onDone}>Done</button>
          <button type="button" className="pfw-cover-check" onClick={onViewProfile}>View on my profile</button>
        </div>
      </div>
    </div>);
}
...
function WebGoLiveStage({ description, onDescriptionChange, onGoLive, onSchedule, onClose }) {
  ...
  return (
    <div className="pfw-golive-stage" style={{ backgroundImage: "url(assets/live-preview-camera.jpg)" }}
      role="dialog" aria-modal="true" aria-label="Go live">
      <div className="pfw-golive-scrim-top" aria-hidden="true"></div>

      <button type="button" className="pfw-golive-back" aria-label="Cancel" onClick={onClose}>
        <IconifyIcon name="lucide:x" size={22} color="#fff" />
      </button>

      <div className="pfw-golive-who">
        <Avatar name={ME.name} src={ME.avatar} size={32} />
        <span className="pfw-golive-name">{ME.name}</span>
        <span className="pfw-golive-dest"><IconifyIcon name="lucide:rss" size={13} color="var(--brand-navy)" />Newsfeed</span>
      </div>

      <div className="pfw-golive-tools">
        {PFW_LIVE_TOOLS.map((t) => (
          <button key={t.label} type="button" className="pfw-golive-tool">
            <span>{t.label}</span>
            <IconifyIcon name={t.icon} size={19} color="#fff" />
          </button>
        ))}
      </div>

      <div className="pfw-golive-scrim-bottom" aria-hidden="true"></div>
      <div className="pfw-golive-bottom">
        {descOpen ? (
          <input autoFocus className="pfw-golive-desc-input" placeholder="Add a description..."
            value={description} onChange={(e) => onDescriptionChange(e.target.value)}
            onBlur={() => setDescOpen(false)} />
        ) : (
          <button type="button" className="pfw-golive-desc-btn" onClick={() => setDescOpen(true)}>
            {description || "Tap to add a description..."}
          </button>
        )}
        <div className="pfw-golive-actions">
          <button type="button" className="pfw-golive-go-btn" onClick={onGoLive}>Go Live</button>
          <button type="button" className="pfw-golive-sched-btn" onClick={onSchedule}>
            <IconifyIcon name="lucide:calendar-clock" size={17} color="#fff" />Schedule
          </button>
        </div>
      </div>
    </div>);
...
function WebBroadcastStage({ onEnd }) {
  ...
  const [chat, setChat] = useState([
    ...
    { n: ME.name, t: "Course + notes for tonight: https://profinity.app/8d-lip-design", me: true },
  ]);
  ...
  const send = () => {
    const val = msg.trim();
    if (!val) return;
    setChat((c) => c.concat({ n: ME.name, t: val, me: true }));
    setMsg("");
  };
  ...
  const onCam = [].concat(
    [{ n: "You", av: liveCam ? "assets/live-preview-camera.jpg" : ME.avatar, me: true, off: !liveCam }],
    guests.map((g) => ({ n: g.n.split(" ")[0], av: g.av }))
  );

  return (
    <div className="pfw-bcast" role="dialog" aria-modal="true" aria-label="Live broadcast">
      {onCam.length > 1 ? (
        <div className={"pfw-bcast-grid n" + Math.min(onCam.length, 4)}>
          {onCam.slice(0, 4).map((p) => (
            <div className={"pfw-bcast-cell" + (p.off ? " camoff" : "")} key={p.n}>
              <img className={p.me && !front ? "rear" : undefined} src={p.av} alt="" />
              {p.off && <span className="co">Camera off</span>}
              <span className="nm">{p.n}{p.me && liveMuted && <IconifyIcon name="lucide:mic-off" size={11} color="#fff" />}</span>
            </div>
          ))}
        </div>
      ) : (
        <React.Fragment>
          <img className={"pfw-bcast-cam" + (front ? "" : " rear") + (liveCam ? "" : " hidden")} src="assets/live-preview-camera.jpg" alt="" />
          {!liveCam &&
            <div className="pfw-bcast-camoff">
      ...
      <div className="pfw-bcast-top">
        <span className="pfw-bcast-dot">LIVE</span>
        <span className="pfw-bcast-clock">{clock}</span>
        <span className="pfw-bcast-viewers"><IconifyIcon name="lucide:eye" size={15} color="#fff" />142</span>
        <button type="button" className="pfw-bcast-guest-btn" aria-label="Add guest" onClick={() => setGuestSheet(true)}>
          <IconifyIcon name="lucide:user-plus" size={16} color="#fff" />
          Add guest
        </button>
        <button type="button" className="pfw-bcast-x" aria-label="End broadcast" onClick={() => setConfirmEnd(true)}>
          <IconifyIcon name="lucide:x" size={19} color="#fff" />
        </button>
      </div>

      <div className="pfw-bcast-chat" ref={chatRef} onScroll={onChatScroll}>
        {chat.map((c, i) => (
          <div className={"pfw-bcast-msg" + (c.me ? " me" : "") + (c.fresh ? " in" : "")} key={i}>
            <Avatar className="pfw-bcast-msg-av" name={c.n} src={pfwBcastAvatar(c)} size={24} />
            <span className="pfw-bcast-msg-tx"><b>{c.n}</b> {isHostMsg(c) ? pfwBcastLinkify(c.t, "pfw-bcast-link") : c.t}</span>
          </div>
        ))}
      </div>

      <div className="pfw-bcast-hearts" aria-hidden="true">
        {hearts.map((h) => (
          <span className="pfw-bcast-heart" key={h.id}
      ...
      {guestSheet &&
        <div className="pfw-bcast-guest" role="dialog" aria-modal="true" aria-label="Invite a guest"
          onClick={(e) => { if (e.target === e.currentTarget) setGuestSheet(false); }}>
          <div className="pfw-bcast-guest-card">
            <h3>Invite a guest</h3>
            <p className="pfw-bcast-guest-p">Guests you bring on become co-hosts — they can invite people too.</p>
            <div className="pfw-bcast-guest-search">
              <IconifyIcon name="lucide:search" size={17} color="var(--gray-450)" />
              <input placeholder="Search users" aria-label="Search users" />
            </div>
            <div className="pfw-bcast-guest-list">
              {PFW_BCAST_GUESTS.map((g) => {
                const on = guests.some((x) => x.u === g.u);
                const pending = !on && invited.indexOf(g.u) !== -1;
                return (
                  <div className="pfw-bcast-guest-row" key={g.u}>
                    <img src={g.av} alt="" />
                    <span className="tx"><b>{g.u}</b><i>{pending ? "Waiting to accept…" : g.f}</i></span>
                    <button type="button" className={"pfw-bcast-guest-invite" + (on ? " on" : "") + (pending ? " pending" : "")}
                      disabled={on || pending}
                      onClick={() => {
            ...
            <button type="button" className="pfw-bcast-guest-link" onClick={() => { setCopied(true); setTimeout(() => setCopied(false), 1800); }}>
              <span className="ic"><IconifyIcon name={copied ? "lucide:check" : "lucide:link"} size={18} color={copied ? "var(--success)" : "var(--brand-navy)"} /></span>
              <span className="tx">
      ...
      {confirmEnd &&
        <div className="pfw-bcast-endc" role="dialog" aria-modal="true" aria-label="End live broadcast"
          onClick={(e) => { if (e.target === e.currentTarget) setConfirmEnd(false); }}>
          <div className="pfw-bcast-endc-card">
            <span className="pfw-bcast-endc-ic"><IconifyIcon name="lucide:radio" size={24} color="var(--error)" /></span>
            ...
            <label className="pfw-bcast-endc-keep">
              <input type="checkbox" checked={keepPost} onChange={(e) => setKeepPost(e.target.checked)} />
              <span className="bx">{keepPost && <IconifyIcon name="lucide:check" size={12} color="#fff" />}</span>
              <span className="tx">
                <b>Post the replay to Newsfeed</b>
                <i>Members who missed it can still watch. Untick to discard the recording.</i>
              </span>
            </label>
            <button type="button" className="pfw-bcast-endc-go" onClick={() => onEnd(keepPost)}>{keepPost ? "End live & post replay" : "End live & discard"}</button>
            <button type="button" className="pfw-bcast-endc-cancel" onClick={() => setConfirmEnd(false)}>Keep streaming</button>
          </div>
        </div>}
    </div>);
}
...
function MediaCarousel({ images, video, aspect, onLoveReact }) {
  ...
  const items = [
    ...(video && video.src ? [{ type: "video", src: video.src, poster: video.cover || video.poster || null, ratio: video.ratio || null }] : []),
    ...
    <div className="mc-wrap">
      <div className={"mc-scroll" + (single ? " mc-scroll-single" + (aspect ? " mc-aspect-" + aspect : "") : "")} ref={ref} onScroll={onScroll}>
        {items.map((it, i) => it.type === "video"
          ? <div key={i} className="mc-img mc-vid" style={single && it.ratio ? { aspectRatio: it.ratio } : undefined}
              aria-label={"Video, item " + (i + 1) + " of " + items.length}>
              <PFMediaVideo src={it.src} poster={it.poster} />
            </div>
          : <img key={i} src={it.src} alt={"Image " + (i + 1) + " of " + items.length} className="mc-img"
          onClick={wrap(() => openFullscreen(i))} />
        )}
      </div>
      {!single && <span className="mc-count">{idx + 1}/{items.length}</span>}
      {!single && <SlidingDots count={items.length} idx={idx} />}
      {heartNode}
      {fs &&
      <div className="sm-fs" onClick={(e) => {e.stopPropagation();}}>
          <div className="mc-fs-track" onScroll={onFsScroll} ref={fsRef}>
            {items.map((it, i) => it.type === "video"
              ? <div key={i} className="mc-fs-vid"><PFMediaVideo src={it.src} poster={it.poster} /></div>
              : <img key={i} src={it.src} alt={"Image " + (i + 1) + " of " + items.length} />)}
          </div>
          <button type="button" className="sm-fs-close" aria-label="Close fullscreen"
        onClick={() => setFs(false)}>
            <IconifyIcon name="lucide:x" size={24} color="var(--white)" />
          </button>
          <span className="mc-fs-count">{fsIdx + 1}/{items.length}</span>
        </div>
      }
    </div>
  );
...
function SampleMedia({ sample, postId, saved, onSave, onReport, onLoveReact, author, likes, commentsCount, shares, liked, comments, onLike, onComment, onShare, caption }) {
  ...
  if (sample.type === "video" && sample.src) {
    return (
      <>
        <div className="sm-video sm-video-real" style={{ aspectRatio: sample.ratio || 16 / 9 }}>
          <PFMediaVideo src={sample.src} poster={sample.poster}
  ...
  return (
    <div className="pf-poll">
      ...
      <div className="pf-poll-opts">
        ...
        disabled={answered}
        onClick={(e) => { setVoted(i); popPoints(e.currentTarget, POLL_POINTS, { label: "Voted in a poll", actionId: "evt_poll_vote", sound: "correct" }); }}>
      ...
      <div className="pf-quiz-opts">
        {q.options.map((o, i) => {
          ...
          return (
            ...
            onClick={(e) => {
              setPicked(i);
              if (o.correct) popPoints(e.currentTarget, QUIZ_POINTS, { label: "Quiz answered correctly", actionId: "evt_quiz_correct", sound: "correct" });
              else popWrong();
            }}>
...
function pinMenuItemsFor(pinned, canPin, onPin, pinScope) {
  if (!canPin) return [];
  return [{ key: "pin", label: pinned ? "Unpin from top" : "Pin to top of " + (pinScope === "channel" ? "channel" : "feed"),
...
function ChannelFeedCard({ post, st, pinned, canPin, onPin, pinScope, onToggleLike, onReact, onDoubleTapLove, onSave, onShare, onAddComment, onAddReply }) {
...
function CourseCommentCard({ post, st, pinned, canPin, onPin, pinScope, onToggleLike, onReact, onSave, onAddComment, onShare }) {
  const [replying, setReplying] = useState(false);
  const [likesOpen, setLikesOpen] = useState(false);
  const [reportedOpen, setReportedOpen] = useState(false);
  const liked = !!st.reaction;
...
window.PFMediaStore = window.PFMediaStore || (function () {
  ...
  function isRef(src) { return typeof src === "string" && src.indexOf("pfmedia:") === 0; }
  function put(blob) {
  ...
  function cached(src) { return urls[src] || null; }
  function url(src) {
  ...
  function clear() {
    return open().then((db) => { db.transaction(STORE, "readwrite").objectStore(STORE).clear(); }).catch(() => {});
  }
  return { put, url, cached, isRef, clear };
})();
  ...
  return new Promise((resolve) => {
    ...
    reader.onload = () => {
      ...
      img.onload = () => {
        ...
        try { resolve(c.toDataURL("image/jpeg", 0.85)); } catch (e) { resolve(raw); }
      };
      img.src = raw;
    };
    reader.readAsDataURL(file);
...
function usePFMediaSrc(src) {
  ...
  useEffect(() => {
    if (!isRef) { setResolved(src); return undefined; }
    let live = true;
    const hit = store.cached(src);
    if (hit) { setResolved(hit); return undefined; }
    setResolved(null);
    store.url(src).then((u) => { if (live) setResolved(u); });
    return () => { live = false; };
  }, [src]);
  return resolved;
}
...
function PFMediaVideo({ src, poster, className, style }) {
  const real = usePFMediaSrc(src);
  return (
    <video src={real || undefined} poster={poster || undefined} controls playsInline preload="metadata"
...
function readUserPosts() {
  try {
    ...
    return list.filter((p) => p && p.author && p.author.name && (p.body || p.sharedPost)).map((p) =>
      ...
      p.video && !p.sample && !(p.media && p.media.length) ? { ...p, sample: { type: "video", poster: p.video.cover || undefined, src: p.video.src, ratio: p.video.ratio } } : p);
  } catch (e) { return []; }
}
...
function FeedUploadCard({ post, onCancel }) {
  ...
  return (
    <div className={"pf-upl" + (done ? " is-done" : "")} role="status" aria-live="polite"
      style={{ background: "var(--surface-card)", borderRadius: "var(--r-md)" }}>
      <div className="pf-upl-row">
        <div className="pf-upl-thumb" aria-hidden="true">
          {cover
            ? <img src={cover} alt="" />
            : <span className="pf-upl-thumb-ph"><IconifyIcon name="lucide:video" size={18} color="var(--brand-navy)" /></span>}
          {post.video && cover && <span className="pf-upl-play"><IconifyIcon name="lucide:play" size={11} color="#fff" /></span>}
          {!post.video && extra > 0 && <span className="pf-upl-count">+{extra}</span>}
          {!done && <span className="pf-upl-dim" style={{ height: (100 - pct) + "%" }} />}
        </div>
        <div className="pf-upl-main">
          <div className="pf-upl-title">{done ? "Posted" : "Uploading your post\u2026"}</div>
          <div className="pf-upl-sub">
            {done
              ? "Your " + uploadMediaLabel(post) + " is live on the feed"
              : <>{post.body ? <span className="pf-upl-snippet">{post.body}</span> : null}<span className="pf-upl-pct">{pct}%</span></>}
          </div>
          <div className="pf-upl-track"><span className="pf-upl-bar" style={{ width: pct + "%" }} /></div>
        </div>
        {done
          ? <span className="pf-upl-check" aria-hidden="true"><IconifyIcon name="lucide:check" size={16} color="#fff" /></span>
          : <button type="button" className="pf-upl-cancel" aria-label="Cancel upload" onClick={onCancel}>
              <IconifyIcon name="lucide:x" size={16} color="var(--text-secondary)" />
            </button>}
      </div>
    </div>);
}
...
function pinnedKeyFor(channel) { return channel ? PF_PINNED_KEY + ":" + channel : PF_PINNED_KEY; }
function pinnedDefaultsFor(channel) { return channel ? CHANNEL_PINNED_DEFAULTS[channel] || [] : PF_PINNED_DEFAULTS; }
...
function writePinnedIds(ids, channel) {
  try { localStorage.setItem(pinnedKeyFor(channel), JSON.stringify(ids)); } catch (e) {}
}
...
function PinnedBox({ items, open, onToggle, expandedIds, onToggleItem, renderItem, innerRef }) {
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver(([e]) => setStuck(!e.isIntersecting && e.boundingClientRect.top < 84), { rootMargin: "-84px 0px 0px 0px", threshold: 0 });
    io.observe(el);
    return () => io.disconnect();
  }, [embed]);
  return (
    <>
    {!embed && <div className="pf-pinned-sentinel" ref={sentinelRef} aria-hidden="true" />}
    <section className={"pf-pinned-box" + (open ? " open" : "") + (embed ? " pf-pinned-box--m" : " sticky") + (stuck ? " stuck" : "")} ref={innerRef} aria-label="Pinned posts">
...
function Feed({ channel } = {}) {
    ...
    const id = setInterval(() => {
      ...
      const next = userPostsRef.current.map((p) => {
        ...
        if (u.doneAt) {
          ...
          changed = true;
          return { ...p, uploading: null };
        }
        if (uploadProgressOf(p) < 1) return p;
        changed = true;
        if (u.reward) rewards.push(u.reward);
        return { ...p, uploading: { ...u, doneAt: now } };
      });
      if (changed) {
        writeUserPosts(next.map((p) => p.uploading ? { ...p, uploading: null } : p));
        setUserPosts(next);
        rewards.forEach((r) => setTimeout(() => popPoints(null, r.amount || POST_POINTS, { label: r.label || "Shared a post", actionId: r.actionId || "evt_create_post", sound: "post" }), 350));
      } else {
        setUploadTick((t) => t + 1);
      }
    }, 100);
  ...
  useEffect(() => {
    let reward = null;
    try { reward = JSON.parse(sessionStorage.getItem(POST_REWARD_KEY)); sessionStorage.removeItem(POST_REWARD_KEY); } catch (e) {}
    if (!reward || !reward.amount) return;
    const t = setTimeout(() => popPoints(null, reward.amount, { label: reward.label || "Shared a post", actionId: reward.actionId || "evt_create_post", sound: "post" }), 700);
    return () => clearTimeout(t);
  }, []);
  ...
  useEffect(() => { setPinnedIds(readPinnedIds(channel)); }, [channel]);
  ...
  const addPost = ({ body, media, video, live, categories, poll, document: doc, location, bg, sharedPost, channelBucket }) => {
    ...
    const sample = video && !mixed ? { type: "video", poster: video.cover, src: video.src, ratio: video.ratio, duration: video.duration } : null;
    const post = { id: "u" + Date.now(), author: { name: ME.name, avatar: ME.avatar, seals: ["gb", "verified"] },
  ...
  const renderFeedItem = ({ item: p, mode, pinned }) => {
    const pinProps = { pinned: !!pinned, canPin, onPin: () => togglePin(p.id), pinScope: channel ? "channel" : "feed" };
        if (mode === "teaser") {
  ...
  return (
    <main className="feed" data-screen-label="Home feed">
      ...
      {userPosts.filter((p) => p.uploading).map((p) => <FeedUploadCard key={p.id} post={p} onCancel={() => cancelUpload(p.id)} />)}
