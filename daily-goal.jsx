/* ===========================================================================
   ...
   =========================================================================== */
const { useState: useStateDG, useEffect: useEffectDG, useRef: useRefDG, useMemo: useMemoDG } = React;
...
function dayKeyDG(d) { const dt = d ? new Date(d) : new Date(); return dt.getFullYear() + "-" + (dt.getMonth() + 1) + "-" + dt.getDate(); }
function todayStatsDG(state) {
  const today = dayKeyDG();
  let points = 0, actions = 0;
  (state.ledger || []).forEach((t) => {
    if (t.pointsDelta > 0 && dayKeyDG(t.ts) === today) { points += t.pointsDelta; actions += 1; }
  });
...
function DgLottie() {
  ...
  useEffectDG(() => {
    ...
    const start = () => {
      ...
      anim = window.lottie.loadAnimation({ container: host.current, renderer: "svg", loop: true, autoplay: true, path: DG_LOTTIE });
    };
  ...
  return <span ref={host} className="dg-lottie" aria-hidden="true" />;
}

/* Gold coin glyph for the Today tile (filled, so it reads as a coin rather
   than an outline) */
function CoinDG({ size }) {
  const sz = size || 18;
  return (
    <svg className="dg-coin" width={sz} height={sz} viewBox="0 0 24 24" aria-hidden="true">
      <defs>
        <linearGradient id="dgCoinG" x1="4" y1="3" x2="20" y2="21" gradientUnits="userSpaceOnUse">
          <stop offset="0" stopColor="#ffe08a" /><stop offset="1" stopColor="#e7a020" />
        </linearGradient>
      </defs>
      <circle cx="12" cy="12" r="10.5" fill="url(#dgCoinG)" />
      <circle cx="12" cy="12" r="7.6" fill="none" stroke="#b8730c" strokeWidth="1.4" strokeOpacity=".75" />
      <text x="12" y="16.2" textAnchor="middle" fontFamily="Georgia, 'Times New Roman', serif" fontWeight="700" fontSize="11.5" fill="#8a5507">P</text>
    </svg>
  );
}
...
function BrandDG({ k }) {
  if (k === "profinity") return <img className="dg-sh-pf" src="assets/profinity-icon-purple-gold.png" alt="" />;
  if (k === "instagram") return (
    <svg viewBox="0 0 24 24" width="30" height="30" aria-hidden="true">
      <rect x="2" y="2" width="20" height="20" rx="5.5" fill="none" stroke="#fff" strokeWidth="2" />
  ...
  if (k === "facebook") return (
    <svg viewBox="0 0 24 24" width="34" height="34" aria-hidden="true">
      <path fill="#fff" d="M13.6 21v-7.9h2.7l.4-3.1h-3.1V8c0-.9.3-1.5 1.6-1.5h1.7V3.7c-.3 0-1.3-.1-2.4-.1-2.4 0-4 1.5-4 4.1V10H7.8v3.1h2.7V21h3.1z" />
    </svg>
  );
  /* messages */
  return (
    <svg viewBox="0 0 24 24" width="30" height="30" aria-hidden="true">
      <path fill="#2e5bff" d="M12 3.5c-4.9 0-8.8 3.4-8.8 7.6 0 2.1 1 4 2.6 5.4L5.2 20l3.9-1.6c.9.3 1.9.4 2.9.4 4.9 0 8.8-3.4 8.8-7.6S16.9 3.5 12 3.5z" />
    </svg>
  );
}

const DG_SHARE_APPS = [
  { k: "profinity", label: "PROfinity" },
...
function DgShareSheet({ first, points, streak, actions, onClose, onPick }) {
  useEffectDG(() => {
  ...
  return (
    <div className="dg-sh" role="dialog" aria-modal="true" aria-label="Share">
      <div className="dg-sh-top">
        <div className="dg-sh-card">
          <p className="dg-sh-say">I’m smashing my daily goal on PROfinity!</p>
          <span className="dg-sh-mascot" aria-hidden="true"><DgLottie /></span>
          <div className="dg-sh-rows">
            <div className="dg-sh-row">
              <span className="dg-sh-ri is-gold"><CoinDG size={22} /></span>
              <span className="dg-sh-rl">Points earned</span>
              <span className="dg-sh-rv">{points}</span>
            </div>
            <div className="dg-sh-row">
              <span className="dg-sh-ri is-green"><DSDG.IconifyIcon name="lucide:flame" size={20} color="#e8611a" /></span>
              <span className="dg-sh-rl">Day streak</span>
              <span className="dg-sh-rv">{streak}</span>
            </div>
            <div className="dg-sh-row">
              <span className="dg-sh-ri is-blue"><DSDG.IconifyIcon name="lucide:check-check" size={20} color="#1d8fd1" /></span>
              <span className="dg-sh-rl">Actions today</span>
              <span className="dg-sh-rv">{actions}</span>
            </div>
          </div>
          <img className="dg-sh-logo" src="assets/profinity-academy-logo-full.png" alt="PROfinity Academy" />
        </div>
        <div className="dg-sh-caption">Goal reached!</div>
      </div>

      <div className="dg-sh-sheet">
        <div className="dg-sh-hd">
          <button type="button" className="dg-sh-x" onClick={onClose} aria-label="Close share"><DSDG.IconifyIcon name="lucide:x" size={22} color="currentColor" /></button>
          <span className="dg-sh-ti">Share</span>
        </div>
        <div className="dg-sh-grid">
          {DG_SHARE_APPS.map((a) => (
            <button key={a.k} type="button" className={"dg-sh-app-btn is-" + a.k} onClick={() => onPick(a.k, a.label)}>
              <span className="dg-sh-circ"><BrandDG k={a.k} /></span>
              <span className="dg-sh-lab">{a.label}</span>
            </button>
          ))}
          <button type="button" className="dg-sh-app-btn is-plain" onClick={() => onPick("save", "Save")}>
            <span className="dg-sh-circ"><DSDG.IconifyIcon name="lucide:download" size={26} color="currentColor" /></span>
            <span className="dg-sh-lab">Save</span>
          </button>
          <button type="button" className="dg-sh-app-btn is-plain" onClick={() => onPick("more", "More")}>
            <span className="dg-sh-circ"><DSDG.IconifyIcon name="lucide:more-horizontal" size={26} color="currentColor" /></span>
            <span className="dg-sh-lab">More</span>
          </button>
        </div>
      </div>
    </div>
...
function DgConfetti() {
  ...
  return <div ref={host} className="dg-confetti" aria-hidden="true" />;
}
...
function DailyGoalScreen() {
  ...
  const { points, actions } = useMemoDG(() => todayStatsDG(state), [state]);
  ...
  const copyShare = () => {
    ...
    let settled = false;
    const done = () => { if (settled) return; settled = true; flash("Copied — paste it anywhere"); };
    if (navigator.clipboard && navigator.clipboard.writeText) { navigator.clipboard.writeText(copy).then(done, done); setTimeout(done, 600); }
    else { try { const ta = document.createElement("textarea"); ta.value = copy; document.body.appendChild(ta); ta.select(); document.execCommand("copy"); ta.remove(); } catch (e) { /* ignore */ } done(); }
  };
  const nativeShare = (fallback) => {
    if (navigator.share) {
      navigator.share({ title: "My PROfinity streak", text: shareText, url: shareUrl })
        .then(() => flash("Shared — nice one"))
  ...
  const captureCard = () => {
    ...
    const rasterMascot = () => new Promise((resolve) => {
      ...
      try {
        ...
        img.onload = () => {
          try {
            const c = document.createElement("canvas"); c.width = Math.round(r.width * 2); c.height = Math.round(r.height * 2);
    ...
    return rasterMascot().then((mascotPng) => {
      ...
      const onclone = (doc) => {
        doc.querySelectorAll(".dg-sh, .dg-sh-card, .dg-sh-card *").forEach((n) => { n.style.animation = "none"; n.style.opacity = "1"; n.style.transform = "none"; });
        const card = doc.querySelector(".dg-sh-card"); if (card) card.style.boxShadow = "none";
        const m = doc.querySelector(".dg-sh-mascot");
        if (m && mascotPng) { m.innerHTML = ""; const im = doc.createElement("img"); im.src = mascotPng; im.style.cssText = "display:block;width:100%;height:100%"; m.appendChild(im); }
      };
      ...
      const squareUp = (c) => {
        const pad = 56, side = Math.max(c.width, c.height) + pad * 2;
        ...
        grad.addColorStop(0, "#332d76"); grad.addColorStop(1, "#14122f");
        g.fillStyle = grad; g.fillRect(0, 0, side, side);
      ...
      return window.html2canvas(el, { backgroundColor: null, scale: 2, useCORS: true, logging: false, onclone })
        .then((c) => squareUp(c).toDataURL("image/png"))
        .catch(() => null);
    });
  };
  ...
  const postToFeed = (img) => {
    ...
    const post = {
      id: "u" + Date.now(),
      author: { name: me, avatar: (state.user && state.user.avatar) || "assets/avatar-katy.jpg", seals: ["gb", "verified"] },
      time: "Just now", hashtags: ["dailygoal"], media: img ? [img] : [], body,
      bg: img ? null : { id: "navy", css: "linear-gradient(150deg,#292569,#3d3688)", fg: "#fff" },
      video: null, live: false, likes: "0", comments: "0", shares: "0", commentList: []
    };
    try {
  ...
  const pickShare = (k, label) => {
    if (k === "profinity") { captureCard().then((img) => { setSharing(false); postToFeed(img); }); return; }
    if (k === "save") {
      captureCard().then((img) => {
        setSharing(false);
        if (img) { try { const a = document.createElement("a"); a.href = img; a.download = "profinity-daily-goal.png"; document.body.appendChild(a); a.click(); a.remove(); } catch (e) { /* ignore */ } }
        flash("Saved to Photos");
      });
      return;
    }
    setSharing(false);
    if (k === "more") { if (!nativeShare(copyShare)) copyShare(); return; }
    if (k === "messages") {
      if (nativeShare(copyShare)) return;
      try { window.location.href = "sms:?&body=" + encodeURIComponent(shareText + " " + shareUrl); } catch (e) { /* ignore */ }
      copyShare();
      return;
    }
    if (!nativeShare(() => { copyShare(); })) {
      const copy = shareText + " " + shareUrl;
      const done = () => flash("Copied — paste it into " + label);
      if (navigator.clipboard && navigator.clipboard.writeText) navigator.clipboard.writeText(copy).then(done, done);
      else copyShare();
    }
  };

  useEffectDG(() => {
    const onKey = (e) => { if (e.key === "Escape" && !sharing) goDG(ret); };
  ...
  useEffectDG(() => {
    try { window.dispatchEvent(new CustomEvent("pf:daily-goal", { detail: { points, mark } })); } catch (e) { /* older WebView */ }
  }, []);
    ...
    <div className="dg-screen" data-screen-label="Daily Goal Reached">
      ...
      <div className="dg-body">
        ...
        <div className="dg-stats" role="list">
          <div className="dg-stat is-gold" role="listitem" style={{ "--d": "120ms" }}>
            <span className="dg-stat-h">Today</span>
            <span className="dg-stat-v"><CoinDG size={19} /> {shown}</span>
          </div>
          <div className="dg-stat is-green" role="listitem" style={{ "--d": "230ms" }}>
            <span className="dg-stat-h">Streak</span>
            <span className="dg-stat-v"><DSDG.IconifyIcon name="lucide:flame" size={18} color="currentColor" /> {streak}<small>{streak === 1 ? " day" : " days"}</small></span>
          </div>
          <div className="dg-stat is-blue" role="listitem" style={{ "--d": "340ms" }}>
            <span className="dg-stat-h">Actions</span>
            <span className="dg-stat-v"><DSDG.IconifyIcon name="lucide:check-check" size={18} color="currentColor" /> {Math.max(actions, 1)}</span>
          </div>
        </div>
      ...
      <div className="dg-foot">
        <button className="dg-btn-share" type="button" onClick={share} aria-label="Share"><DSDG.IconifyIcon name="lucide:share" size={20} color="currentColor" /></button>
        <button className="dg-btn-cta" type="button" onClick={() => goDG(ret)}>Keep earning <DSDG.IconifyIcon name="lucide:arrow-right" size={18} color="currentColor" /></button>
      </div>

      {sharing && <DgShareSheet first={first} points={points} streak={streak} actions={Math.max(actions, 1)} onClose={() => setSharing(false)} onPick={pickShare} />}
      {toast && <div className="ml-toast dg-toast" role="status"><DSDG.IconifyIcon name="lucide:check" size={16} color="#fff" />{toast}</div>}
    </div>
  );
}
...
function useDeviceScaleDG() {
  ...
  useEffectDG(() => { const u = () => setScale(calc()); window.addEventListener("resize", u); return () => window.removeEventListener("resize", u); }, []);
  return scale;
}
function useIsMobileDG() {
  const [mobile, setMobile] = useStateDG(() => window.matchMedia("(max-width:768px)").matches);
  useEffectDG(() => { const mq = window.matchMedia("(max-width:768px)"); const h = (e) => setMobile(e.matches); mq.addEventListener("change", h); return () => mq.removeEventListener("change", h); }, []);
  return mobile;
}
...
ReactDOM.createRoot(document.getElementById("pf-root")).render(<DailyGoalApp />);
