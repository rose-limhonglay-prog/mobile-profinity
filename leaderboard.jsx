...
const LB_FIELD = [
  { name: "Marcus Webb", points: 9840, trend: "up" },
  { name: "Eleanor Pena", points: 8120, trend: "same" },
  { name: "Sofia Alarcón", points: 6790, trend: "up" },
  { name: "Jonas Adeyemi", points: 5310, trend: "down" },
  { name: "Grace Lindqvist", points: 4980, trend: "up" },
  { name: "Hana Kobayashi", points: 4400, trend: "same" },
  { name: "Ravi Chandran", points: 3920, trend: "down" },
  { name: "Olivia Marsh", points: 3510, trend: "up" },
  { name: "Deniz Aydın", points: 3105, trend: "same" },
  { name: "Priya Nandwani", points: 2640, trend: "down" },
  { name: "Liam O'Connor", points: 2210, trend: "up" },
  { name: "Amara Okafor", points: 1890, trend: "same" },
  { name: "Ben Fischer", points: 1420, trend: "down" },
  { name: "Nadia Petrova", points: 990, trend: "up" }
];
...
function LeaderboardScreen() {
  ...
  const ranked = useMemoLB(() => {
    const rows = LB_FIELD.map((r) => ({ ...r, isKaty: false })).concat([{ name: state.user.name + " Moore", points: state.rollingPoints30, trend: "up", isKaty: true }]);
  ...
  return (
    <div className="ml-screen lb-screen" data-screen-label="Leaderboard">
      ...
      <div className="ml-scroll">
        <div className="lb-window-note">Rolling 30-day points · resets continuously</div>

        <div className="lb-podium">
          {[top3[1], top3[0], top3[2]].map((r, idx) => (
            <div key={r.name} className={"lb-podium-item rank-" + r.rank + (r.isKaty ? " is-katy" : "")}>
              <span className="lb-podium-avatar">{initials(r.name)}</span>
              <span className="lb-podium-rank">#{r.rank}</span>
              <span className="lb-podium-name">{r.name}</span>
              <span className="lb-podium-pts">{PF_LB.formatNumber(r.points)} pts</span>
            </div>
          ))}
        </div>

        <div className="ml-sec-h"><h2>Top 15</h2></div>
        <div className="ml-card" style={{ padding: 0 }}>
          {rest.map((r) => (
            <div key={r.name} className={"lb-row" + (r.isKaty ? " is-katy" : "")}>
              <span className="lb-row-rank">#{r.rank}</span>
              <span className="lb-row-avatar">{initials(r.name)}</span>
              <span className="lb-row-main">
                <span className="ti">{r.name}{r.isKaty && " (You)"}</span>
                <span className="su">{prizeFor(r.rank) || "—"}</span>
              </span>
              <span className="lb-row-pts">{PF_LB.formatNumber(r.points)}</span>
              <DSLB.IconifyIcon name={r.trend === "up" ? "lucide:trending-up" : r.trend === "down" ? "lucide:trending-down" : "lucide:minus"}
                size={16} color={r.trend === "up" ? "var(--success)" : r.trend === "down" ? "var(--error)" : "var(--gray-400)"} />
            </div>
          ))}
        </div>
      </div>
