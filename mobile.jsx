...
function ptsLottieDataM() {
  if (!ptsLottiePromiseM) {
    ptsLottiePromiseM = fetch(PTS_LOTTIE_SRCM).then((r) => r.ok ? r.json() : null).catch(() => { ptsLottiePromiseM = null; return null; });
  }
...
function ptsReadFullM() {
  const eng = window.PFLoyalty;
  if (eng && eng.getConfig) { try { return Math.max(1, +eng.getConfig().beakerFullPoints || PTS_FULL_FALLBACKM); } catch (e) { /* fall through */ } }
...
function usePointsCountUpM(target) {
  ...
  useEffectM(() => {
    ...
    if (ptsReduceMotionM() || typeof requestAnimationFrame !== "function") { from.current = target; setShown(target); return; }
...
function PointsTallyM({ host, on, shown, bump, delta }) {
  return ReactDOM.createPortal(
    <div className={"m-pts-tally" + (on ? " on" : "")} aria-hidden="true">
      <button type="button" tabIndex={-1} className={"m-pts" + (delta && on ? " earn" : "")} onClick={() => go("RewardsDashboard.html")}>
        <PointsIconM />
...
function PointsPillM() {
  ...
  useEffectM(() => {
    const refresh = () => { setTotal(ptsReadTotalM()); setFull(ptsReadFullM()); };
    const onEarn = (e) => {
      ...
      if (eng) {
        try {
          if (eng.awardPoints) eng.awardPoints(amt, e.detail.label, e.detail.actionId);
  ...
  return (
    <React.Fragment>
      ...
      {tallyHost && <PointsTallyM host={tallyHost} on={tally} shown={shown} bump={bump} delta={delta} />}
    </React.Fragment>);
}
