...
function pmLeagueStandings() {
  ...
  const me = { name: PM_ME.name + " (You)", avatar: PM_ME.avatar, points: mine };
  if (window.PFLeague && window.PFLeague.getStandings) return window.PFLeague.getStandings(me);
  const rows = PM_LEAGUE_FALLBACK.map((r) => ({ ...r, isMe: false })).concat([{ ...me, isMe: true }])
...
const PMTabBar = React.forwardRef(function PMTabBar({ compact, active = "Profile" }, ref) {
  return (
    <nav ref={ref} className={"pm-tabs" + (compact ? " pm-tabs-compact" : "")} aria-label="Primary">
      {PM_TABS.map((t) =>
      <button key={t.key} className={"pm-tab" + (t.key === active ? " on" : "")}
      aria-current={t.key === active ? "page" : undefined}
      onClick={() => { if (t.href) goPM(t.href); else if (t.key === "Profile" && active !== "Profile") goPM("ProfileMobile.html"); }}>
          <span className="ic">
            <DSPM.IconifyIcon name={t.icon} size={20} color={t.key === active ? "#fff" : "var(--gray-450)"} />
...
function pmLoadBanners() {
  ...
  return Array.isArray(b) ? b.filter(k => { const it = pmBannerItem(k); return it && (!it.needsValue || conn[k]); }) : [];
}
...
function pmBannerItem(key) { return PM_BANNER_ITEMS.find(it => it.key === key); }
...
function pmBannerText(key, conn) {
  const it = pmBannerItem(key); if (!it) return "";
  const raw = conn[key];
...
const PM_CONNECT_NETWORKS = {
  instagram: { app: "https://www.instagram.com/", brand: "Instagram", login: "sign in to Instagram", what: "username", privacy: "Your Instagram messages and password stay private. Profinity only sees the username you choose to show.", def: () => pmDefaultSocialHandle("instagram") },
  threads:   { app: "https://www.threads.net/", brand: "Threads", login: "sign in to Threads", what: "username", privacy: "Your Threads messages and password stay private. Profinity only sees the username you choose to show.", def: () => pmDefaultSocialHandle("threads") },
  facebook:  { app: "https://www.facebook.com/", brand: "Facebook", login: "sign in to Facebook", what: "profile name", privacy: "Your Facebook messages, friends and password stay private. Profinity only sees your public profile name.", def: () => pmDefaultSocialHandle("facebook") },
  whatsapp:  { app: "https://wa.me/", brand: "WhatsApp", login: "open WhatsApp", what: "number", privacy: "Your personal messages and calls on WhatsApp stay end-to-end encrypted. No one, not even Profinity, can read or listen to them.", contact: "Your WhatsApp number is never public without your permission and your contacts aren't shared with Profinity.", alert: "People who tap on your banner can message you. Your number won't show on Profinity until you confirm it in WhatsApp. Go to WhatsApp to add your banner.", def: () => (PM_ME.clinicNumber || "").trim() || "+44 7700 900000" },
  linkedin:  { app: "https://www.linkedin.com/", brand: "LinkedIn", login: "sign in to LinkedIn", what: "profile name", privacy: "Your LinkedIn messages and connections stay private. Profinity only sees your public profile name.", def: () => pmDefaultSocialHandle("linkedin") },
  twitter:   { app: "https://x.com/", brand: "X", login: "sign in to X", what: "username", privacy: "Your X messages and password stay private. Profinity only sees the username you choose to show.", def: () => pmDefaultSocialHandle("twitter") },
  youtube:   { app: "https://www.youtube.com/", brand: "YouTube", login: "sign in to YouTube", what: "channel", privacy: "Your YouTube account and watch history stay private. Profinity only sees your public channel name.", def: () => PM_ME.name || "" },
};
function pmConnectNetwork(key) { return PM_CONNECT_NETWORKS[key] || null; }
...
function PMConnectAccountScreen({ item, conn, onDone, onClose }) {
  ...
  React.useEffect(() => { if (editing) { const t = setTimeout(() => { try { inputRef.current && inputRef.current.focus(); } catch (e) {} }, 60); return () => clearTimeout(t); } }, [editing]);
  if (!net) return null;
  ...
  function Av({ badgeKey, className }) {
    ...
    return (
      <span className={"pm-cac-av" + (className ? " " + className : "")}>
        <DSPM.Avatar name={PM_ME.name} src={PM_ME.avatar} size={56} />
        <span className="pm-cac-badge">{bi ? <DSPM.IconifyIcon name={bi.icon} size={15} color={bi.color} /> : <b>P</b>}</span>
      </span>
    );
  }
  ...
  if (step !== "confirm") {
    ...
    return (
      <div className="pm-cac-alert-overlay" onClick={onClose}>
        <div className="pm-cac-alert" role="alertdialog" aria-modal="true" onClick={e => e.stopPropagation()}>
          <h2>Add {net.brand} to profile</h2>
          <p>{waiting
          ...
          {waiting
            ? <button type="button" className="pm-cac-alert-go" onClick={() => setStep("confirm")}>I've signed in to {net.brand}</button>
            : <a className="pm-cac-alert-go" href={net.app} target="_blank" rel="noopener noreferrer" onClick={() => setStep("waiting")}>Go to {net.brand}</a>}
          <button type="button" className="pm-cac-alert-cancel" onClick={onClose}>Cancel</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="pm-cac" role="dialog" aria-modal="true" aria-label={"Connect " + net.brand}>
      <button type="button" className="pm-cac-close" aria-label="Close" onClick={onClose}>
        <DSPM.IconifyIcon name="lucide:x" size={26} color="var(--pm-bn-ink)" />
      </button>
      <div className="pm-cac-body">
        <p className="pm-cac-eyebrow">Add {net.brand} to Connected accounts</p>
        <h1 className="pm-cac-title">Add {an} {net.brand} banner to this profile</h1>

        <div className="pm-cac-card">
          <h2>Connected accounts</h2>
          <div className="pm-cac-row">
            <Av badgeKey={item.key} className="new" />
            <span className="pm-cac-dots" aria-hidden="true">{[0,1,2,3,4,5].map(i => <i key={i} />)}</span>
            <span className="pm-cac-stack">
              <Av className="me" />
              {others.map(k => <Av key={k} badgeKey={k} />)}
            </span>
          </div>
          <div className="pm-cac-acct">
            {editing ? (
              <span className="pm-cac-input">
              ...
              <>
                <span className="pm-cac-acct-nm"><DSPM.IconifyIcon name="lucide:check" size={14} color="#25a244" />{shown || ("Add your " + net.what)}</span>
                <button type="button" className="pm-cac-acct-edit" onClick={() => setEditing(true)}>{shown ? "Not you?" : "Add"}</button>
              </>
            )}
          </div>
        </div>

        <ul className="pm-cac-points">
          <li>
            <DSPM.IconifyIcon name="lucide:circle-user-round" size={28} color="var(--pm-bn-ink)" />
          ...
          <li>
            <DSPM.IconifyIcon name={item.key === "whatsapp" ? "lucide:phone" : "lucide:at-sign"} size={28} color="var(--pm-bn-ink)" />
          ...
          <li>
            <DSPM.IconifyIcon name="lucide:lock" size={28} color="var(--pm-bn-ink)" />
            <span>{net.privacy}</span>
          </li>
        </ul>
      </div>
      <div className="pm-cac-foot">
        <p className="pm-cac-fine">We'll <b>show your {net.brand} {net.what}</b> as a banner under your bio. You can remove it from Banners anytime.</p>
        <button type="button" className="pm-cac-confirm" disabled={!clean} onClick={() => onDone(clean)}>Confirm</button>
        <button type="button" className="pm-cac-cancel" onClick={onClose}>Cancel</button>
      </div>
    </div>
  );
}
...
function SocialStep({ onComplete }) {
  ...
  const [handles, setHandles] = useStatePM(() => {
    const conn = pmLoadSocialConnections(); const h = {};
    PM_SOCIALS.forEach(s => { h[s.key] = conn[s.key] || ""; });
    return h;
  });
  ...
  function save() {
    ...
    filled.forEach(s => { if (!banners.includes(s.key)) banners.push(s.key); });
    pmSaveBanners(banners);
    onComplete();
  }
  return (
    <div className="pm-sheet-step">
      <p className="pm-sheet-desc">Add your username for the profiles you want to show. Leave any blank to skip it.</p>
      <div className="pm-sheet-socials">
        {PM_SOCIALS.map(s => {
          const isName = s.key === "facebook" || s.key === "linkedin";
          const v = handles[s.key] || "";
          return (
            <label key={s.key} className={"pm-sheet-social" + (v.trim() ? " filled" : "")}>
              <DSPM.IconifyIcon name={s.icon} size={28} color={s.color} />
              <span className="pm-sheet-social-nm">{s.label}<small>Optional</small></span>
              <span className="pm-sheet-social-field">
                {!isName && <span className="pm-sheet-social-at" aria-hidden="true">@</span>}
                <input type="text" value={v} placeholder={isName ? "Profile name" : "username"}
                  autoCapitalize={isName ? "words" : "none"} autoCorrect="off" spellCheck={false}
                  onChange={e => setHandles(prev => ({ ...prev, [s.key]: e.target.value.replace(isName ? /^\s+/ : /^@+/, "") }))} />
                {v && <button type="button" className="pm-sheet-social-clear" aria-label={"Clear " + s.label} onClick={() => setHandles(prev => ({ ...prev, [s.key]: "" }))}>
                  <DSPM.IconifyIcon name="lucide:x" size={12} color="#fff" />
                </button>}
              </span>
            </label>
          );
      ...
      <button className="pm-sheet-cta" disabled={filled.length === 0} onClick={save}>
...
const PM_COUNTRY_ABBR = {
  "united kingdom": "UK", "great britain": "UK", "england": "UK", "scotland": "UK", "wales": "UK", "northern ireland": "UK",
  "united states": "USA", "united states of america": "USA", "australia": "AU", "canada": "CA", "ireland": "IE",
  "new zealand": "NZ", "united arab emirates": "UAE", "south africa": "ZA", "germany": "DE", "france": "FR", "spain": "ES",
  "italy": "IT", "netherlands": "NL", "singapore": "SG", "india": "IN", "pakistan": "PK", "philippines": "PH", "portugal": "PT"
};
...
function PMPlaceChips({ location, clinic }) {
  if (!location && !clinic) return null;
  return (
    <span className="pm-chip-pair">
      {location && <span className="pm-chip pin" title={location}><DSPM.IconifyIcon name="lucide:map-pin" size={16} color="var(--brand-navy)" /><span className="tx">{pmShortLocation(location)}</span></span>}
      {clinic && <span className="pm-chip pin" title={clinic}><DSPM.IconifyIcon name="lucide:building-2" size={16} color="var(--brand-navy)" /><span className="tx">{clinic}</span></span>}
    </span>);
}

function PMChipRows({ children, maxRows = 2, trailing = null, className = "" }) {
  const items = React.Children.toArray(children).filter(Boolean);
  const ref = React.useRef(null);
  const [expanded, setExpanded] = useStatePM(false);
  const [visible, setVisible] = useStatePM(items.length);
  const key = items.length;
  useEffectPM(() => { setVisible(items.length); }, [key]);
  useEffectPM(() => {
    const onResize = () => { setVisible(items.length); };
  ...
  React.useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      const rows = [];
      tops.slice().sort((a, b) => a - b).forEach((t) => { if (!rows.length || t - rows[rows.length - 1] > 8) rows.push(t); });
  ...
  return (
    <div className={"pm-ig-chiprow " + className} ref={ref}>
      {shown.map((it, i) => <span className="pm-chip-item" data-chip-item="1" key={it.key != null ? it.key : i}>{it}</span>)}
      {hidden > 0 &&
      <button type="button" className="pm-chip more" aria-label={"Show " + hidden + " more"} onClick={() => setExpanded(true)}>+{hidden}</button>}
      {trailing}
    </div>);
}
...
const PM_FOLLOW_POOL = [
  { name: "Dr Tim Pearce", handle: "drtimpearce", avatar: "assets/avatar-drtim.png" },
  { name: "Miranda Pearce", handle: "mirandapearce", avatar: "assets/avatar-miranda.jpg" },
  { name: "Priya Shah", handle: "priyashah.aesthetics", avatar: "assets/avatar-priya-shah.jpg" },
  { name: "Nurse Beth", handle: "nursebeth", avatar: "assets/avatar-nurse-beth.jpg" },
  { name: "Dr Amir Khan", handle: "dramirkhan", avatar: "assets/avatar-amir-khan.jpg" },
  { name: "Dr. Sarah Collins", handle: "drsarahcollins", avatar: "assets/avatar-sarah-collins.jpg" },
  { name: "Mark Ellis", handle: "markellis.clinics", avatar: "assets/avatar-mark-ellis.jpg" }];
function PMFollowedBy({ user, from }) {
  ...
  let h = 0; for (let i = 0; i < user.name.length; i++) h = (h * 31 + user.name.charCodeAt(i)) >>> 0;
  ...
  const open = (m) => {
    ...
    const q = "?id=" + encodeURIComponent(PL ? PL.slug(m.name) : m.name.toLowerCase().replace(/[^a-z0-9]+/g, "-")) +
      "&name=" + encodeURIComponent(m.name) + "&avatar=" + encodeURIComponent(m.avatar) + (from ? "&from=" + encodeURIComponent(from) : "");
    goPM("ProfileMobile.html" + q);
  };
  return (
    <div className="pm-followed">
      <span className="pm-followed-avs" aria-hidden="true">
        {picks.map((m) => <span className="pm-followed-av" key={m.handle}><DSPM.Avatar name={m.name} src={m.avatar} size={30} /></span>)}
      </span>
      <p className="pm-followed-tx">
        Followed by {named.map((m, i) =>
          <React.Fragment key={m.handle}>
            {i > 0 && (others > 0 ? ", " : " and ")}
            <button type="button" className="pm-followed-nm" onClick={() => open(m)}>{m.handle}</button>
          </React.Fragment>)}
...
function openDirectMessagePM(user) {
  ...
  try { q.set("from", (window.location.pathname.split("/").pop() || "ProfileMobile.html") + (window.location.search || "")); } catch (e) { q.set("from", "ProfileMobile.html"); }
  goPM("DirectMessage.html?" + q.toString());
}

function OtherProfileScreen({ user, from }) {
  ...
  return (
    <div className="pm-screen" data-screen-label={"Profile — " + user.name}>
      <OtherProfileTopBar name={user.name} onBack={() => goPM(backTo)} onMessage={() => openDirectMessagePM(user)} />
      {avatarOpen && (
      ...
      {qrOpen && <PMQrShareScreen user={user} link={pmProfileLinkPM(user)} onClose={() => setQrOpen(false)} onShare={() => setShareOpen(true)} />}
      {shareOpen && (
        <PMShareProfileSheet user={user} link={pmProfileLinkPM(user)} onClose={() => setShareOpen(false)}
      ...
      <div className="pm-scroll" ref={scrollRef}>
        <div className="pm-ig">
          ...
          <div className="pm-ig-name">
            ...
            {/* no role beside the name — the headline line below already says it */}
            {user.seals && <DSPM.VerificationSeals seals={user.seals} size={20} />}
          </div>

          {/* Same bio block as the owner's profile: flag + headline,
              specialties, upcoming live, free-text bio — each line only
              when the member has that fact. */}
          {(user.headline || user.flag || user.role || (user.specialties && user.specialties.length) || user.upcomingLive || user.bio) &&
          <div className="pm-ig-bio">
            {(user.headline || user.flag || user.role) && <p>{user.flag && <span className="bi">{user.flag}</span>} {user.headline || user.role}</p>}
            {user.specialties && user.specialties.length > 0 && <p><span className="bi">💉</span> {user.specialties.join(" · ")}</p>}
            {user.upcomingLive &&
            <p className="pm-ig-live">
              <span className="bi"><DSPM.IconifyIcon name="lucide:calendar" size={14} color="var(--text-primary)" /></span>
              Upcoming Live: <b>{user.upcomingLive}</b>
            </p>}
            {user.bio && <p>{user.bio}</p>}
          </div>}
          {user.link &&
          <a className="pm-ig-link" href={/^https?:/.test(user.link) ? user.link : "https://" + user.link} target="_blank" rel="noopener noreferrer">
            <DSPM.IconifyIcon name="lucide:link" size={17} color="var(--ai-purple)" />{user.link.replace(/^https?:\/\//, "")}
          </a>}

          <PMFollowedBy user={user} from={from} />

          {(user.location || user.clinic || (user.banners && Object.keys(user.banners).length > 0)) &&
          <PMChipRows>
            <PMPlaceChips key="place" location={user.location} clinic={user.clinic} />
            {Object.keys(user.banners || {}).map((key) => {
              const it = pmBannerItem(key); if (!it) return null;
              /* Static chip: the handle isn't a verified account, so it never links out. */
              return (
                <span key={key} className="pm-banner static">
                  <DSPM.IconifyIcon name={it.icon} size={17} color={it.color} />{pmBannerText(key, user.banners)}
                </span>);
            })}
          </PMChipRows>}
          ...
          <div className="pm-ig-actions">
            ...
            <button className="pm-ig-btn" onClick={() => openDirectMessagePM(user)}>
              <DSPM.IconifyIcon name="lucide:message-circle" size={16} color="var(--text-heading)" />Message
            </button>
            <button className="pm-ig-btn icon" aria-label="Share profile" onClick={() => setShareOpen(true)}>
              <DSPM.IconifyIcon name="lucide:share-2" size={20} color="var(--text-heading)" />
            </button>
          </div>
        </div>

        {/* "Shared with you" card intentionally dropped (2026-09-16) — the
            Followed-by row in the header covers mutual connections. */}
      ...
      <PMTabBar compact={chromeHidden} active={activeTab} />
    </div>);
...
function PMEditRow({ label, value, onChange, options, placeholder, multiline, stack, inputMode, className }) {
...
function PMBannerValueSheet({ item, initial, onSave, onClose }) {
  const [val, setVal] = useStatePM(initial || "");
  const inputRef = React.useRef(null);
  React.useEffect(() => { const t = setTimeout(() => { try { inputRef.current && inputRef.current.focus(); } catch (e) {} }, 120); return () => clearTimeout(t); }, []);
  const clean = val.trim();
  function submit(e) { e && e.preventDefault(); if (!clean) return; onSave(clean); }
  return (
    <div className="pm-sheet-overlay pm-bnv-overlay" onClick={onClose}>
      <form className="pm-sheet pm-bnv" onClick={e => e.stopPropagation()} onSubmit={submit}>
        <div className="pm-sheet-drag" />
        <div className="pm-bnv-hd">
          <span className="pm-bnv-ic"><DSPM.IconifyIcon name={item.icon} size={26} color="var(--text-heading)" /></span>
          <h2>{item.prompt || item.label}</h2>
        </div>
        <div className="pm-bnv-field">
          ...
          {val && <button type="button" className="pm-bnv-clear" aria-label="Clear" onClick={() => setVal("")}><DSPM.IconifyIcon name="lucide:x" size={14} color="#fff" /></button>}
        </div>
        {item.key === "custom" && <p className="pm-bnv-hint">A short line that shows as a banner under your bio.</p>}
        <div className="pm-bnv-acts">
          <button type="button" className="pm-bnv-cancel" onClick={onClose}>Cancel</button>
          <button type="submit" className="pm-bnv-save" disabled={!clean}>{initial ? "Save" : "Add"}</button>
        </div>
      </form>
    </div>
  );
...
function PMBannersScreen({ onBack }) {
  ...
  function saveValue(key, value) {
    const next = { ...pmLoadSocialConnections(), [key]: value };
    pmSaveJSON(PM_SOCIAL_CONN_KEY, next); setConn(next);
  }
  /* ?connect=instagram(&step=confirm) opens the connect flow straight away — demo / QA entry. */
  const [connect, setConnect] = useStatePM(() => {
    try { const k = new URLSearchParams(window.location.search).get("connect"); return k && pmConnectNetwork(k) ? k : null; } catch (e) { return null; }
  }); /* banner key being connected via its app */
  function add(key) {
    const it = pmBannerItem(key); if (!it || banners.includes(key)) return;
    if (pmConnectNetwork(key)) { setConnect(key); return; }
    ...
    <div className="pm-edit-screen pm-bn-screen" data-screen-label="Banners">
      <header className="pm-edit-hd">
        <button type="button" className="pm-edit-back" aria-label="Back" onClick={onBack}>
          <DSPM.IconifyIcon name="lucide:chevron-left" size={24} color="var(--pm-bn-ink)" />
        </button>
        <h1>Banners</h1>
      ...
      <div className="pm-edit-body">
        <section className="pm-bn-sec">
          ...
          {banners.length === 0 && (
            <p className="pm-bn-empty">Nothing on your profile yet. Add something below and it shows up under your bio.</p>
          )}
            ...
            {banners.map(key => {
              ...
              return (
                <div key={key} className={"pm-bn-row" + (dragKey === key ? " dragging" : "")}>
                  <button type="button" className="pm-bn-handle" aria-label={"Reorder " + it.label} onPointerDown={(e) => onHandleDown(e, key)}>
                    <DSPM.IconifyIcon name="lucide:menu" size={24} color="var(--pm-bn-ink)" />
                  </button>
                  <span className="pm-bn-ic"><DSPM.IconifyIcon name={it.icon} size={26} color="var(--pm-bn-ink)" /></span>
                  <span className="pm-bn-nm">{pmBannerText(key, conn)}</span>
                  {it.editable && (
                    <button type="button" className="pm-bn-x pm-bn-edit" aria-label={"Edit " + it.label} onClick={() => setSheet({ key, mode: "edit" })}>
                      <DSPM.IconifyIcon name="lucide:pencil" size={22} color="var(--pm-bn-ink)" />
                    </button>
                  )}
                  <button type="button" className="pm-bn-x" aria-label={"Remove " + it.label} onClick={() => remove(key)}>
                    <DSPM.IconifyIcon name="lucide:x" size={24} color="var(--pm-bn-ink)" />
                  </button>
                </div>
              );
            })}
        ...
        <section className="pm-bn-sec">
          ...
          {addable.length === 0 && (
            <p className="pm-bn-empty">Everything is already on your profile.</p>
          )}
          <div className="pm-bn-list">
            {addable.map(it => (
              <button key={it.key} type="button" className="pm-bn-row add" onClick={() => add(it.key)}>
                <span className="pm-bn-handle"><DSPM.IconifyIcon name="lucide:circle-plus" size={24} color="var(--pm-bn-ink)" /></span>
                <span className="pm-bn-ic"><DSPM.IconifyIcon name={it.icon} size={26} color="var(--pm-bn-ink)" /></span>
                <span className="pm-bn-nm">{it.label}</span>
              </button>
            ))}
          </div>
        </section>
      ...
      {sheetItem && (
        <PMBannerValueSheet item={sheetItem} initial={sheet.mode === "edit" ? (conn[sheet.key] || "") : (conn[sheet.key] || "")}
          onSave={onSheetSave} onClose={() => setSheet(null)} />
      )}
      {connect && pmBannerItem(connect) && (
        <PMConnectAccountScreen item={pmBannerItem(connect)} conn={conn} onDone={onConnected} onClose={() => setConnect(null)} />
      )}
    </div>
  );
}
...
/* One Personal Goal question: numbered badge (tick once answered), the
   ...
   type. */
function PMGoalField({ index, question, value, onChange }) {
  const taRef = React.useRef(null);
  useAutoGrowPM(taRef, value);
  const answered = !!(value && value.trim());
  return (
    <div className={"pm-goal-card" + (answered ? " answered" : "")}>
      <div className="pm-goal-top">
        <span className="pm-goal-num" aria-hidden="true">
          {answered ? <DSPM.IconifyIcon name="lucide:check" size={15} color="#fff" /> : index + 1}
        </span>
        <label className="pm-goal-q" htmlFor={"pm-goal-" + index}>{question}</label>
      </div>
      <div className="pm-goal-box">
        <textarea id={"pm-goal-" + index} ref={taRef} rows={2} className="pm-goal-input" value={value}
...
function PMEditProfileScreen({ profile, onCancel, onSave, openBanners }) {
    ...
    <div className="pm-edit-screen" data-screen-label="Edit profile">
      ...
      <div className="pm-edit-body">
        ...
        <section className="pm-edit-group">
          ...
          <PMEditRow label="Tell us about yourself" multiline className="pm-edit-row-bio" value={form.bio} onChange={setField("bio")} placeholder="Tell us about yourself" />
...
function PMShareProfileSheet({ user, own, link, onClose, onQr }) {
  const [caption, setCaption] = useStatePM(() => own
  ...
  function go(k) {
    ...
    switch (k) {
      case "messages": goPM("Messages.html?share=" + u); return;
  ...
  function postToFeed() {
    ...
    const post = {
      id: "u" + Date.now(),
      author: { name: PM_ME.name, avatar: PM_ME.avatar, seals: ["gb", "verified"] },
  ...
  return (
    <div className="pm-sheet-overlay pm-shp-overlay" onClick={onClose}>
      <div className="pm-sheet pm-shp" role="dialog" aria-modal="true" aria-label="Share profile" onClick={e => e.stopPropagation()}>
        <div className="pm-sheet-drag" />
        <div className="pm-shp-hd">
          <h3>Share profile</h3>
          <button type="button" className="pm-shp-x" aria-label="Close" onClick={onClose}>
            <DSPM.IconifyIcon name="lucide:x" size={20} color="var(--text-heading)" />
          </button>
        </div>
        <div className="pm-shp-body">
          <div className="pm-shp-card">
            <DSPM.Avatar name={user.name} src={user.avatar} size={52} />
            <div className="pm-shp-card-tx">
              <span className="nm">{user.name}</span>
              <span className="rl">{user.role || "PROfinity member"}</span>
              <span className="hd">{handle}</span>
            </div>
            <span className="pm-shp-card-badge"><img src="assets/profinity-icon-purple-gold.png" alt="" /></span>
          </div>

          <section className="pm-shp-feed">
            <div className="pm-shp-feed-hd">
              <span className="pm-shp-feed-ic"><DSPM.IconifyIcon name="lucide:rss" size={17} color="#fff" /></span>
              <div>
                <b>Share to Newsfeed</b>
                <i>Posts a link card to everyone who follows you</i>
              </div>
            </div>
            <textarea className="pm-shp-caption" rows={2} value={caption} placeholder="Say something about this profile…"
              onChange={e => setCaption(e.target.value)} />
            <button type="button" className={"pm-shp-post" + (posted ? " done" : "")} onClick={postToFeed} disabled={posted}>
              <DSPM.IconifyIcon name={posted ? "lucide:check" : "lucide:send"} size={17} color="#fff" />
              {posted ? "Shared" : "Post to Newsfeed"}
            </button>
          </section>

          <div className="pm-shp-sec-lb">Share to</div>
          <div className="pm-shp-rail">
            {PM_SHARE_TARGETS.map(t => (
              <button key={t.k} type="button" className="pm-shp-tile" onClick={() => go(t.k)}>
                <span className="pm-shp-tile-ic" style={{ background: t.bg }}>
                  <DSPM.IconifyIcon name={t.icon} size={24} color={t.color} />
                </span>
                <span className="pm-shp-tile-lb">{t.label}</span>
              </button>
            ))}
          </div>
        </div>
        {toast && <div className="pm-shp-toast" role="status">{toast}</div>}
      </div>
    </div>
  );
}
...
function PMQrShareScreen({ user, link, onClose, onShare }) {
  const [toast, setToast] = useStatePM("");
  ...
  async function download() {
    ...
    try {
      ...
      ctx.fillStyle = "#111"; ctx.font = "600 48px Poppins, system-ui, sans-serif"; ctx.textAlign = "center";
    ...
    <div className="pm-qrs" role="dialog" aria-modal="true" aria-label="Profile QR code">
      <header className="pm-qrs-hd">
        <button type="button" className="pm-qrs-round" aria-label="Close" onClick={onClose}>
          <DSPM.IconifyIcon name="lucide:x" size={26} color="var(--pm-qr-ink)" />
        </button>
        <h2 className="pm-qrs-title">Share Profile</h2>
        <span className="pm-qrs-round pm-qrs-spacer" aria-hidden="true" />
      </header>
      <div className="pm-qrs-mid">
        ...
        <div className="pm-qrs-tiles">
          <button type="button" className="pm-qrs-tile" onClick={share}>
            <DSPM.IconifyIcon name="lucide:share" size={28} color="var(--pm-qr-ink)" /><span>Share profile</span>
          </button>
          <button type="button" className="pm-qrs-tile" onClick={copyLink}>
            <DSPM.IconifyIcon name="lucide:link" size={28} color="var(--pm-qr-ink)" /><span>Copy link</span>
          </button>
          <button type="button" className="pm-qrs-tile" onClick={download}>
            <DSPM.IconifyIcon name="lucide:download" size={28} color="var(--pm-qr-ink)" /><span>Download</span>
          </button>
        </div>
      </div>
...
function PMAvatarViewer({ user, own, following, onToggleFollow, onChangeAvatar, onQr, onShare, onClose }) {
  const [toast, setToast] = useStatePM("");
  const fileRef = React.useRef(null);
...
function PMScreen() {
  ...
  const [shareOpen, setShareOpen] = useStatePM(() => {
    try { return new URLSearchParams(window.location.search).get("share") === "1"; } catch (e) { return false; }
  });
  ...
  if (editOpen) {
    return <PMEditProfileScreen profile={profile} openBanners={editBanners}
      onCancel={() => { setEditOpen(false); setEditBanners(false); }}
      onSave={(u) => { saveProfileEdits(u); setEditBanners(false); }} />;
  }
  ...
  return (
    <div className={"pm-screen" + (chromeFloat ? " chrome-float" : "") + (chromeHidden ? " chrome-hidden" : "")} data-screen-label="Profile (mobile)">
          ...
          {qrOpen && <PMQrShareScreen user={m} link={pmProfileLinkPM(null)} onClose={() => setQrOpen(false)} onShare={() => setShareOpen(true)} />}
          {shareOpen && (
            <PMShareProfileSheet user={m} own link={pmProfileLinkPM(null)} onClose={() => setShareOpen(false)}
              onQr={() => { setShareOpen(false); setQrOpen(true); }} />
          )}
          <div className="pm-scroll" ref={scrollRef}>
            <div className="pm-ig">
              ...
              <PMChipRows trailing={
                <button type="button" className="pm-chip add" onClick={() => { setEditBanners(true); setEditOpen(true); }}>
                  <DSPM.IconifyIcon name="lucide:plus" size={16} color="var(--gray-500)" />Add
                </button>}>
                <PMPlaceChips key="place" location={m.location} clinic={m.clinic} />
                {banners.map(key => {
                  const it = pmBannerItem(key); if (!it) return null;
                  /* Static chip: the handle isn't a verified account, so it never links out. */
                  return (
                    <span key={key} className="pm-banner static">
                      <DSPM.IconifyIcon name={it.icon} size={17} color={it.color} />{pmBannerText(key, socialConn)}
                    </span>
                  );
                })}
              </PMChipRows>
...
function ProfileMobileApp() {
  ...
  const { id: idParam, name: nameParam, avatar: avatarParam, role: roleParam, from: fromParam } = readProfileIdParamPM();
  const otherUser = idParam ?
  PM_OTHER_USERS[idParam] || (nameParam ? buildMinimalProfilePM(nameParam, avatarParam, roleParam) : null) :
  null;
  const content = idParam ?
  otherUser ? <OtherProfileScreen user={otherUser} from={fromParam} /> : <ProfileNotFoundScreen /> :
