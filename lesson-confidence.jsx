...
function LXRelated() {
  ...
  const related = PFLS_LX.pickRelated({ purchased, tier: LX_TIER, n: 3, includeOwned: true, exclude: [LX_COURSE.slug] });
      ...
      <div className="lc-related">
        ...
        <button type="button" className="lc-course" key={c.title} onClick={() => goLX(courseUrlLX({ ...c, price }))}>
            ...
            <span className="lc-course-tx">
              <span className="lc-course-eyebrow">{included ? "Included in your membership" : price ? c.level + " · Paid course" : "Course"}</span>
              <span className="lc-course-title" style={{ display: "block" }}>{c.title}</span>
              <span className="lc-course-meta">{c.lessons} lessons · {c.dur}</span>
              {price > 0 &&
...
function LXUpgrade({ course }) {
  ...
  const picks = PFLS_LX.CATALOG.filter((c) => !c.owned && c.slug !== course.slug && !PFLS_LX.includedIn(c.slug, LX_TIER))
    .sort((a, b) => (b.cat === cat) - (a.cat === cat)).slice(0, 3);
  return (
    <section className="lc-upgrade" data-screen-label={"Unlock more with " + name}>
      <span className="lc-upgrade-eyebrow">Recommended upgrade</span>
      <h3 className="lc-upgrade-title">Unlock more with {name}</h3>
      <p className="lc-upgrade-body">{next === "mastery" ? "Every course in the catalogue, live case reviews and the Mastery community channels." : "More courses, live events and community perks."}</p>
      {picks.length > 0 &&
      <ul className="lc-upgrade-list">
        {picks.map((c) =>
        <li key={c.slug}>
          <img src={c.image} alt="" />
          <span className="lc-upgrade-tx"><span className="lc-upgrade-name">{c.title}</span><span className="lc-upgrade-meta">{c.lessons} lessons · {c.by}</span></span>
          <DSLX.IconifyIcon name="lucide:lock" size={14} color="rgba(255,255,255,.8)" />
        </li>)}
      </ul>}
      <button type="button" className="lc-upgrade-btn" onClick={() => goLX("MembershipTier.html")}>
        See what's included
        <DSLX.IconifyIcon name="lucide:arrow-right" size={16} color="#7A4A03" />
      </button>
    </section>);
}
  ...
  return (
    <div className={"lc-screen" + (LX_LIGHT ? " lc-light" : "")} data-screen-label={"Course Detail · Confidence (" + (LX_LIGHT ? "light" : "dark") + ")"}>
      ...
      <div className="lc-scroll" ref={scrollRef}>
        ...
        {/* overall course completion (user, 2026-09-15) — the same strip the player uses */}
        {!locked && <LXCourseProgress course={course} flat={flat} done={done} onOpen={() => scrollRef.current && scrollRef.current.scrollTo({ top: 0, behavior: "smooth" })} />}
